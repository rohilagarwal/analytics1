...
abline(v=c(3,5, min(df$X), max(df$X)),h=c(6,10, min(df$Y), max(df$Y)), col=c('red','blue','green','yellow')) # few straight lines at x & y axis. v is vertical and h is horizontal
range(df$X)
#Model
...
predict((fit1), newdata1=data.frame(x))
fitted(fit1)
...
summary(fit1)
coef(fit1)
(Y = coef(fit1)[1] + coef(fit1)[2] * 4) #instead of typing manually, we are making the process automatic
#using model
range(df$X)
(new1 = data.frame(X=c(4,5,3,2))) #new values within range
...
plot(fit1)
par(mfrow=c(2,2)) #drawing multiple frame rows i.e. all graphs in same window
plot(fit1)
par(mfrow=c(1,1))
plot(fit1)
plot(fit1, which=1) #printing 1st plot
