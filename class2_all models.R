...
#Linear Regression
head(women) #head returns top 6 rows
head(mtcars)
#predict weight for new height
plot(women)
plot(women, xlim=c(0, max(women$height)), ylim=c(0, max(women$weight)))
?c
fit1 = lm(weight ~ height, data=women)
summary(fit1)
range(women$height)
cbind(range(women$height))
(ndata1=data.frame(height=c(59.5, 62.5)))
(predicted1 = predict(fit1, newdata= ndata1)) #predicting weights for new entered heights
cbind(ndata1, predicted1) #printing it in columns format
